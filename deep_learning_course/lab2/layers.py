...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        """
        N = x.shape[0]
        log_probs = np.log(softmax(x, axis=1))  # softmax function imported from scipy
        y_class_idx = np.argmax(y, axis=1)
        loss = np.mean(-log_probs[np.arange(N), y_class_idx])
        return loss
...
class L2Regularizer():
    ...
    def backward_params(self):
        ...
        """
        grad_weights = self.weight_decay * 2 * self.weights
